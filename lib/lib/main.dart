...
import 'package:codexhub01/services/call_manger.dart';
...
class _CodeHubAppState extends State<CodeHubApp> {
  ...
  final CallManager _callManager = CallManager(); // Add CallManager instance
  ...
  if (!isOfflineMode && mounted) {
    _callManager.initializeCallListener();
  }
});
  ...
  void dispose() {
    ...
    _callManager.dispose(); // Dispose call manager
    super.dispose();
  }
